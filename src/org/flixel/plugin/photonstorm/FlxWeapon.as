...
package org.flixel.plugin.photonstorm 
{
	...
	public class FlxWeapon 
	{
		 ...
		 */
		private function runFire(method:uint, x:int = 0, y:int = 0, target:FlxSprite = null, angle:int = 0, forceFire:Boolean = false):Boolean
		{
			if (fireRate > 0 && (getTimer() < nextFire) && !forceFire)
			{
				return false;
			}
		 ...
		 * @return	true if a bullet was fired or false if one wasn't available. A reference to the bullet fired is stored in FlxWeapon.currentBullet.
		 */
		public function fireFromAngle(angle:int, forceFire:Boolean = false):Boolean
		{
			return runFire(FIRE_FROM_ANGLE, 0, 0, null, angle, forceFire);
		}
